...
@schedule_review_bp.route('/review/<int:match_id>/resolve_conflict', methods=['GET', 'POST'])
@login_required
@role_required('admin', 'scheduling')
def resolve_conflict(match_id):
    db = get_db()
    c = db.cursor()
    ...
    match = c.fetchone()
    if not match:
        flash("Conflict entry not found.", "danger")
    ...
    if request.method == 'POST':
        ...
        if not selected_mdoc:
            flash("Please select a resident to resolve this conflict.", "warning")
            return redirect(url_for('schedule_review.resolve_conflict', match_id=match_id))

        c.execute("SELECT id FROM schedule_groups WHERE name = ?", (match['block_title'],))
        group = c.fetchone()
        if not group:
            flash(f"Schedule group '{match['block_title']}' not found.", "danger")
            return redirect(url_for("schedule_review.review_matches"))
        ...
        try:
            ...
            db.commit()
            flash("✅ Conflict resolved and match approved.", "success")
            return redirect(url_for("schedule_review.review_matches"))
        except Exception as e:
            db.rollback()
    ...
    return render_template("schedule/resolve_conflict.html", match=match, residents=residents)
