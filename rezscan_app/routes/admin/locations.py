...
def manage_locations():
    ...
    try:
        with get_db() as conn:
            ...
            log_audit_action(
                username, 'view', 'locations', f"Viewed {len(locations)} locations"
            )
...
def delete_location(location_id):
    ...
    try:
        with get_db() as conn:
            ...
            if deleted_rows > 0:
                log_audit_action(
                    username, 'delete_location', 'locations', f"Deleted location: {location_name} (ID: {location_id})"
                )
                logger.info(f"User {username} deleted location: {location_name} (ID: {location_id})")
                flash("Location deleted successfully.", "success")
            else:
                log_audit_action(
                    username, 'delete_location_failed', 'locations', f"Location ID {location_id} not found"
                )
